...
ビュー ID は **bit-packed 整数** `((sceneId << 8) | localId)`。文字列名 `"normal/sample2a"` 等はログ可読化用 (`NavigationTable.nameOf(id)`)。
...
実装の構成は [`docs/design.md`](docs/design.md) と一致している。`QulLoaderNavigation/` 配下の主要ファイル:

- `Main.qml` — Window、物理→仮想キー変換、SceneSlot ペア (§3-2, §8-4)
- `NavigationTable.qml` / `Mediator.qml` / `TransitionManager.qml` / `KeyDispatcher.qml` / `Logger.qml` — singleton 群 (§5–§9)
- `SceneBase.qml` — Scene の共通骨格 (scene-filtered binding、ViewSlot ペア、入力吸収転送 §9-10)
- `OpeningScene.qml` / `NormalScene.qml` / `ClosingScene.qml` — SceneBase 派生
- `ViewBase.qml` — 標準 view の共通骨格 (§9-10, §9-11)
- `OpeningView.qml` / `HomeView.qml` / `MenuView.qml` / `Sample1View.qml` / `Sample2View.qml` / `ClosingView.qml` — view 6 種
  - **`Sample2View.qml` は 1 ファイルで 2 つの ID (`sample2a` / `sample2b`) を担当** する例（同一 QML 多重 ID）
